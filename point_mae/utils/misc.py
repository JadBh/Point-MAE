...
def get_ptcloud_img(ptcloud, roll, pitch, draw_axes=True, axis_length=1.0):
    ...
    lower_bound = min(np.min(ptcloud), 0.0)
    upper_bound = max(np.max(ptcloud), axis_length)

    ax.set_xbound(lower_bound, upper_bound)
    ax.set_ybound(lower_bound, upper_bound)
    ax.set_zbound(lower_bound, upper_bound)
    ...
    if draw_axes:
        ...
        # vertical axis is data Y and its depth axis is data Z.

        axis_definitions = [
            ((axis_length, 0, 0), "red", "X"),
            ((0, axis_length, 0), "blue", "Z"),
            ((0, 0, axis_length), "green", "Y"),
        ]
        for (end_x, end_y, end_z), color, label in axis_definitions:
            ax.plot([0, end_x], [0, end_y], [0, end_z], color=color, linewidth=2)
            ax.text(end_x, end_y, end_z, label, color=color, fontsize=12)
