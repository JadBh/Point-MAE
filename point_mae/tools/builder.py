...
# dataloader
from point_mae.datasets import build_dataset_from_cfg
from point_mae.models import build_model_from_cfg

# utils
from point_mae.utils.logger import *
from point_mae.utils.misc import *
...
def dataset_builder(args, dataset_config):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=dataset_config.others.bs,
        shuffle=shuffle,
...
def save_checkpoint(
    ...
    if args.local_rank == 0:
        torch.save(
            ...
            os.path.join(args.experiment_output_path, prefix + ".pth"),
        )
        print_log(
            f"Save checkpoint at {os.path.join(args.experiment_output_path, prefix + '.pth')}",
            logger=logger,
        )
...
def load_decoder_weights(base_model, ckpt_path, logger=None):
    """Load the decoder half of a full Point-MAE checkpoint into a Point_MAE_Decoder.

    The checkpoint holds the 12-block MAE encoder and the mask token, which
    Point_MAE_Decoder does not define. Those keys are dropped so that the remaining
    ...
    unwrapped_weights = {}
    for checkpoint_key, tensor in checkpoint_weights.items():
        unwrapped_weights[checkpoint_key.removeprefix("module.")] = tensor

    # Keep only what this model declares; the encoder keys are not errors here.
    expected_keys = set(base_model.state_dict().keys())
    decoder_weights = {}
    skipped_keys = []
    for key, tensor in unwrapped_weights.items():
        if key in expected_keys:
            decoder_weights[key] = tensor
        else:
            skipped_keys.append(key)

    base_model.load_state_dict(decoder_weights, strict=True)
    print_log(
        f"Loaded {len(decoder_weights)} tensors, skipped {len(skipped_keys)} encoder tensors",
        logger=logger,
    )
